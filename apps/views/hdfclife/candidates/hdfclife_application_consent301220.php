   ...
   <body>
      ...
      <script>
		...
		//  alert('alert');
		  ...
		  //$("input.agree").prop("disabled", false);
		  
		 // $("input.agree").removeAttr("disabled");
	 ...
	 $(".send_otp").click(function(){
		 ...
		 if(check_len>=10){
			...
			$.get('<?php echo base_url();?>hdfclife/candidates/send_otp', function( data ) {
				//alert(data);
				if(data=="0"){
					alert("Mobile number is not associated this application.");
				}
				else if(data=="6"){
					alert("Your mobile number verification OTP limit is exceeded for today.");
				}
				else if(data=="2"){
	 ...
	 $("#otp").keyup(function(){
		...
		if((get_number!='') && (otp_num_length>=5)){

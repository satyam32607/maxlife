     ...
     /*       $acc_opening_date_valid='0';
        ...
        /*   $candrow=get_table_info('candidates','candidate_id',$this->input->post("candidate_id"));
          ...
          if(strtotime($acc_opening_date) > strtotime($this->input->post("transaction_date")))
		    ...
		    $this->db->where("DATE_FORMAT(transaction_date,'%Y-%m-%d') >=",substr($this->input->post("transaction_date"),0,10));
        	$querybtrans = $this->db->get();
             //echo "--->".$this->db->last_query();
            // die();
             //if($querybtrans->num_rows() > 0 ){
		       // $error_msg='Transaction date is always greater than previous records.';
                  //  echo "-------error_msg------1------->".$error_msg; 
                 //die();
              //}
             //else
			 if($trans_date_valid=='1'){
		        $error_msg='This Transaction is already exists. Please check.';
              }
              else{
      ...
      public function delete($bank_transaction_id,$candidate_id)
	 {
	  ...
	  {	 
        ...
        elseif($this->uri->segment('4'))
		 ...
		 $openingbalance = $this->transactions_model->get_statement_balance($candidate_id,$start_date,$enddate);

		 $results = $this->transactions_model->get_statement($candidate_id,$startdate,$enddate);
         ...
         $firstresults = $this->transactions_model->get_statement_withlimit($candidate_id,$startdate,$enddate,'0',$firstpagelimit);
         $pendingresults = $this->transactions_model->get_statement_withlimit($candidate_id,$startdate,$enddate,$firstpagelimit,$secondpagelimit); 
		 ...
		 */
         ...
         elseif($candrow->bank_id=='4')
		 $this->load->view('admin/transactions/indusind_bank_statement.php', $data);  
         elseif($candrow->bank_id=='5')
		 $this->load->view('admin/transactions/icici_bank_statement.php', $data);    
		 else
		 $this->load->view('admin/transactions/yesbank_statement.php', $data);	     

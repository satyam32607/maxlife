...
public function __construct()
   ...
   public function candidate_training_data_update()
	{   
		    $this->posdb=$this->load->database('pos_db', TRUE); 
			$this->posdb->select('candidates.batch_id,candidates.candidate_id');
			$this->posdb->join('batches', 'batches.batch_id = candidates.batch_id');
			$this->posdb->join('sponsers', 'sponsers.sponser_id = batches.sponser_id');
			$this->posdb->where('sponsers.sponser_id','149');
			$this->posdb->where('candidates.course_complete','1');
			$this->posdb->where('batches.batch_start_date >=','2022-04-01');
			//$this->posdb->limit(1);
			...
			foreach ($result as $row) {
				  ...
				  $this->posdb->select('*');
				  $this->posdb->from('log_final_test');
				  $this->posdb->where('candidate_id', $candidate_id);
				  $this->posdb->order_by('final_test_id', 'ASC');
				  $finaltestquery = $this->posdb->get();
				  if($finaltestquery->num_rows()>0){
			      $testresult = $finaltestquery->result();
			      foreach ($testresult as $ftestrow) {
					  
					 $time_spent =  $this->sec_to_time(rand(1200,3200));
					  $update_data =array( 
					   ...
					   echo "</pre>";
					  $this->posdb->where('candidate_id', $candidate_id);
					  $this->posdb->where('final_test_id', $ftestrow->final_test_id);
					  $updateresult = $this->posdb->update('log_final_test', $update_data);
					 echo "<br><br>";
					 echo $this->posdb->last_query();
				  }
				 }

	 ...
	 function refresh_data_status($campaign_id)
		{
			...
			{	
				...
				$this->db->where('campaign_id',$campaign_id);
				$this->db->limit('200000');
				$this->db->order_by('cand_call_detail_id', 'ASC');
				$query = $this->db->get();
				$results = $query->result();
				$total=0;
				foreach ($results as $rowlog) {
			    ...
			    $this->db->where('cand_call_detail_id',$rowlog->cand_call_detail_id);
			    $updateresult = $this->db->update('candidate_call_details', $updatesstdata);
				$total++;
			}
			
			 if($total>0)
		     $this->session->set_flashdata('success_message', 'Total\'s ('.$total.') Records\'s reset successfully.');
			}
			if($result)
			  return '1';
			 else 

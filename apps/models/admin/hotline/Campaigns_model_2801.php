	   ...
	   function update_data_new($campaign_id){
			...
			$error=0;
			 foreach ($result as $row) {
					...
					$this->db->where('campaign_id',$campaign_id);
					$this->db->where('candidate_status',NULL);
				    ...
				    if($total_records >=$camprow->maximum_calls)
				    {
					    ...
					    /*$this->db->select('slot_id,slot_start_date,duration');
						$this->db->from('campaign_slots');
						$this->db->where('campaign_id',$campaign_id);
						$this->db->where('slot_status','0');
						$this->db->order_by('slot_id','RANDOM');
						$slotlogquery = $this->db->get();
						$slotlogrow = $slotlogquery->row();	
						$slot_start_date = $slotlogrow->slot_start_date;
						$slot_id = $slotlogrow->slot_id;
						$slot_call_duration = $slotlogrow->duration;
						...
						$sec_diff=rand(1,4);
						$slot_start_date = date('Y-m-d H:i:s', strtotime($slotrow->slot_end_date)+$sec_diff);
						$slot_end_date = date('Y-m-d H:i:s', strtotime($slot_start_date)+$slotrow->duration);
				   ...
				   {/*
						...
						$slotlogrow = $slotlogquery->row();	
						$slot_start_date = $slotlogrow->slot_start_date;
						$slot_end_date = date('Y-m-d H:i:s', strtotime($slot_start_date)+$slotlogrow->duration);
						$slot_id = $slotlogrow->slot_id;
						$slot_call_duration = $slotlogrow->duration;*/

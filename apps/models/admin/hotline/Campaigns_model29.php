	 ...
	 function update_call_status($campaign_id)
		{
		  ...
		  if($total_update_records <$camprow->maximum_calls)
			...
			{
			   /* $this->db->select('cand_call_detail_id,end_datetime');
			    $this->db->from('candidate_call_details');
			    $this->db->where('campaign_id', $campaign_id);
				$this->db->where('candidate_status','1');
				$this->db->order_by('cand_call_detail_id', 'DESC');
			    $this->db->limit(1);
			    $lastquery = $this->db->get();
			 	$lastrow = $lastquery->row();*/
			  
				$call_duration=rand(1,$duration);
				$slot_start_date = $camprow->start_date;
				
		/*		$stagerow=get_table_info('hotline_campaigns','campaign_id',$campaign_id);
				$campaignstage = $stagerow->campaign_stage;*/
				//$new_call_duration='';

...
class Projects_model extends CI_Model
{
	 function add()
     {
		...
		$projects_rights=$this->input->post('projects_rights');
		$projects_rights = json_encode($projects_rights);
	...
	function view_projects()
		{
			...
			$this->db->where('users.user_type', 'P');
			...
			foreach ($result as $row) {
				...
				$row->total_branches= $this->count_branches($row->user_id);
		...
		function count_branches($user_id)
		{
	...
	function project_edit($user_id)
		{
			if ($user_id == '') {
				redirect(base_url() . "admin/projects/view");
			}
			$this->db->select('users.*,user_address.country_id,user_address.country_code,user_address.state_id,user_address.city_id,user_address.pin_code,user_address.address');
			$this->db->from('users');
			$this->db->join('user_address', 'user_address.user_address_id = users.user_address_id');
		 ...
		 function update_project($user_id,$user_address_id)
		 {
			$projects_rights=$this->input->post('projects_rights');
			$projects_rights = json_encode($projects_rights);
			$user_modules=$this->input->post('user_modules');

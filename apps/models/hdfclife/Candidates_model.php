...
class Candidates_model extends CI_Model
{
	...
	function check_otp_verfiy_limit($mobile_no){
		 $this->db->where('mobile_no',$mobile_no);
		 $this->db->where('post_date >=', date('Y-m-d').' 00:00:00');
	 	 $this->db->where('post_date <=', date('Y-m-d').' 23:59:59');
		 $this->db->order_by('mob_otp_id','DESC');
		 $query = $this->db->get('mobile_otp');	
		 $result=$query->num_rows();
		 if($result>0){
	  ...
	  function update_action_status($action_type)
       {
		   if($action_type=='pdf')
		   {
			 $data = array(
		   ...
		   $this->db->where('candidate_id',$this->session->userdata('cand_id'));
			$result = $this->db->update('consent_candidates', $data);
			if($result)
			  return '1';
			 else 
			 return '0';
